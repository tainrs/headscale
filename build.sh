...
docker buildx build --build-arg BUILDKIT_INLINE_CACHE=1 --platform "linux/${1}" -f "./linux-${1}.Dockerfile" -t "${org}/${image}-${1}" $(for i in $(jq -r 'to_entries[] | [(.key | ascii_upcase),.value] | join("=")' < VERSION.json); do out+="--build-arg $i " ; done; echo $out; out="") .
